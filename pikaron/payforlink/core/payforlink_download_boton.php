...
class payforlink_download_boton
{
    ...
    */
    ...
    function main($urlcodec)
    {
        ...
        $ganar       = (int) ($precio/2);
        $theme_path  = generate_board_url()."/ext/pikaron/payforlink/styles/all/theme/" . $this->user->lang_name . '/';
        ...
        if (($precio < 1) or ($precio > 150) or (substr($url, 0, 4) != "http"))
        {
            ...
            if ((!$this->config['payforlink_show']) and ($this->db_tools->sql_column_exists(USERS_TABLE, 'user_points')))
            {
